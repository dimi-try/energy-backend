...
1️⃣ Скопировать папку uploads из контейнера на сервер
```
docker cp energy-backend-1:/app/uploads /image-backup/
```

2️⃣ Скопировать с сервера на локальную машину
...
1️⃣ Скопировать изображения с локалки на сервер
...
6️⃣ Выходим с сервера:

```
exit
```

7️⃣ Скачиваем дамп на локальную машину:
...
8️⃣ Подключаемся снова к серверу и чистим временные файлы:
